...
def preprocess_spotify_tracks_ds() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    spotify_tracks_df.drop(["Unnamed: 0", "track_id", "track_name", "album_name", "artists"], axis=1, inplace=True)
    spotify_tracks_df.rename(columns={"track_genre": "genre"}, inplace=True)
    spotify_tracks_df["genre"] = LabelEncoder().fit_transform(spotify_tracks_df["genre"])
    ...
    embedded_cols = {n: len(col.cat.categories) for n, col in spotify_tracks_df.items() if col.dtype == 'category' and len(col.cat.categories) > 2}
    del embedded_cols["genre"]
    X_train, X_test, y_train, y_test = train_test_split(spotify_tracks_df.drop(columns=["genre"]), spotify_tracks_df.loc[:, ["genre"]],
...
class SpotifyTracksDataset(Dataset):
    ...
    def __init__(self, X: pd.DataFrame, y: pd.DataFrame, embedded_col_names: dict, transform=None) -> None:
        """
        Args:
            X: input_data
            y: labels
